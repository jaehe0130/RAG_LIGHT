...
def run_generator():
    ...
    while len(dataset) < TARGET_COUNT:
        ...
        prompt = (
            ...
            f"- 질문 텍스트 내에 구체적인 피심인 기업명(예: 특정 회사명 등)을 직접 노출하지 말고, 일상 민원 형태로 써주세요.\n"
            f"- '일반 국민' 페르소나는 일상 거래에서 겪는 직관적인 민원을 구어체로 질문하고, 답변은 쉽고 친근한 언어로 공정거래 상식을 설명해야 합니다.\n"
            f"- '논문 준비 학생' 페르소나는 학술적/법리적인 깊은 질문을 해야 하며, 답변 또한 판례번호나 공정위 의결서 조항을 명시한 학술적·분석적 언어로 분석해야 합니다.\n"
            f"- '청장년층' 페르소나는 인터넷 인강 환불, 헬스장 위약금, 다이어트 보조제 과장광고, 다크패턴(가짜 타이머, 자동체크 안심결제) 등 디지털 환경에서 겪는 실생활 피해 위주로 질문해야 합니다.\n"
            f"- 답변(answer) 작성 시 반드시 실제 서비스되는 AI 챗봇의 톤앤매너와 구조를 준수해야 합니다.\n"
            f"  [답변 표준 구조]\n"
            f"  1. 도입부: 질문 쟁점에 대해 자연스럽게 대답을 시작하는 연결형 문장. (예: '제시해주신 사례와 관련 규정을 바탕으로 판단할 때, ... 검토가 가능합니다.')\n"
            f"  2. 본문 문단(2~3개): 각 문단은 반드시 '• **[대주제]**' 형식으로 시작하며 그 아래에 상세 법리 및 구제안 기재. 세부 항목은 '- [상세 내용]' 사용.\n"
            f"  3. 결론 문단: 반드시 '• **결론**' 형식으로 시작하며 종합 판단 및 대안 권장.\n"
            f"  4. 종결부: 구체적 검토나 증빙 확인을 권장하는 부드러운 종결 멘트.\n"
            f"  [답변 구조 예시 (헬스장 환불 분쟁)]\n"
            f"  \"제시해주신 사례와 방문판매법을 바탕으로 판단할 때, 헬스장 계약 중도 해지 및 환불 요청 건은 다음과 같이 법적 검토가 가능합니다.\\n\\n• **중도 해지 권리 및 위약금 기준**\\n소비자는 계약 기간 중 언제든지 계약을 해지할 수 있으며, 이 경우 사업자는 총 결제 금액의 10% 범위 내에서 위약금을 청구할 수 있습니다. 헬스장의 '환불 절대 불가' 약관은 약관법 제6조에 따라 고객에게 부당하게 불리한 조항으로 무효에 해당합니다.\\n\\n• **미이용 일수에 대한 환불 의무**\\n사업자는 위약금(10%)과 실제 이용한 일수에 비례한 금액을 제외한 나머지 잔여 금액을 소비자에게 반환해야 합니다. 이를 거부하는 행위는 방문판매법 위반에 해당합니다.\\n\\n• **결론**\\n자체 약관을 이유로 환불을 거부하는 것은 법적 효력이 없으므로, 소비자는 **위약금 10%와 이용 일수만큼의 금액을 제한 잔액에 대해 환불을 요구할 권리**가 있습니다.\\n\\n구체적인 사안에 대해 더 자세한 검토가 필요하다면, 계약 시 작성한 계약서 내용과 실제 이용 일수 등 세부 정보를 확인하는 것이 좋습니다.\"\n"
